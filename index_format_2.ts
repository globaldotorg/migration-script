...
const createUser = (userData: User) => {
  ...
  return clerkClient.users.createUser({
    ...
    publicMetadata: {
      ...
      agreedTerms: userData.agreed_terms,
...
const updateUser = async (userData: User) => {
  ...
  if (response && response.data.length > 0) {
    ...
    if (
      existing.externalId !== userData.id ||
      existing.firstName !== firstName ||
      existing.lastName !== lastName ||
      existing.publicMetadata.agreedTerms !== userData.agreed_terms ||
...
async function processUserToClerk(userData: User, spinner: Ora) {
  ...
  try {
    ...
    appendLog({ id: userData.id, ...error });
  }
}
